...
export default function useOKXOrderBook() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        const updates = msg?.data?.[0];

        if (!updates) return;

        if (updates.bids) {
          bidsRef.current = updateOrderBook(bidsRef.current, updates.bids, true);
        }
        if (updates.asks) {
          asksRef.current = updateOrderBook(asksRef.current, updates.asks, false);
        }
      } catch (err) {
        setHasError(true);
      }
